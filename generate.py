...
import wan.utils.device as device_utils
...
def _parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Disable MPS fallback by setting PYTORCH_ENABLE_MPS_FALLBACK=0.",
    )
    parser.add_argument(
        "--dist_backend",
        type=str,
...
def generate(args):
    with torch.inference_mode():
        ...
        if torch.backends.mps.is_available():
            if args.no_mps_fallback:
                os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "0"
                _MPS_FALLBACK_MSG = "PYTORCH_ENABLE_MPS_FALLBACK set to 0."
            else:
                _mps_env = os.environ.get("PYTORCH_ENABLE_MPS_FALLBACK")
                if _mps_env is None:
                    os.environ.setdefault("PYTORCH_ENABLE_MPS_FALLBACK", "1")
                    _MPS_FALLBACK_MSG = (
                        "PYTORCH_ENABLE_MPS_FALLBACK was not set. Defaulting to 1."
                    )
                else:
                    _MPS_FALLBACK_MSG = (
                        f"PYTORCH_ENABLE_MPS_FALLBACK already set to {_mps_env}."
                    )
        else:
        ...
        device_utils.empty_device_cache()
        device_utils.synchronize_device()
        if dist.is_initialized():
            dist.barrier()
